<template>
  <div class="glass-panel h-full w-full border-white/10 bg-white/5 p-6 lg:p-8">
    <div class="flex flex-col gap-2 border-b border-white/10 pb-6 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <p class="text-xs uppercase tracking-[0.35em] text-white/50">即时上传</p>
        <h3 class="mt-2 text-2xl font-semibold text-white">AstrNest 媒体管理</h3>
        <p class="mt-2 text-sm text-white/70">
          拖拽、选择或按 Ctrl+V 粘贴即可完成上传，支持高清图片与短视频。
        </p>
      </div>
      <div class="rounded-2xl bg-white/5 px-4 py-3 text-right">
        <p class="text-xs text-white/60">待上传</p>
    ...
    <div class="mt-6 space-y-6">
      <div
        ...
        :class="dropActive
          ? 'border-brand-primary bg-white/10 shadow-card'
          : 'border-white/20 bg-surface-body/40 hover:border-brand-primary/70'"
      >
        <UploadCloud class="h-12 w-12 text-brand-primary" />
        <div class="space-y-2">
          <p class="text-lg font-semibold text-white">拖入文件或点击选择</p>
          <p class="text-sm text-white/60">
            支持 JPG / PNG / GIF / WEBP 以及 MP4 / WEBM，单次最多 30 个文件，粘贴前请先聚焦此区域。
          </p>
        </div>
        <div class="flex flex-wrap justify-center gap-3">
          ...
          <button
            ...
            class="inline-flex items-center gap-2 rounded-full bg-white/10 px-5 py-2 text-sm font-semibold text-white transition hover:bg-white/20"
          >
      ...
      <div v-if="!auth.isAuthenticated" class="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
      ...
      <div v-if="selectedFiles.length" class="space-y-4 rounded-3xl border border-white/10 bg-white/5 p-5">
        <div class="flex items-center justify-between text-sm text-white/70">
          <p>已选择 {{ selectedFiles.length }} 个 · {{ totalSizeLabel }}</p>
          <button type="button" class="text-white/60 transition hover:text-white" @click="clearSelection">
            清空所选
          </button>
        </div>
        <ul class="space-y-3">
          <li
            ...
            :key="`${file.name}-${index}`"
            class="flex items-center justify-between rounded-2xl border border-white/5 bg-surface-body/60 px-4 py-3"
          >
            <div class="flex items-center gap-3 text-left">
              <component :is="file.type?.startsWith('video/') ? Film : FileImage" class="h-4 w-4 text-brand-primary" />
              <div>
                <p class="text-sm font-medium text-white">{{ file.name || '剪贴板文件' }}</p>
                <p class="text-xs text-white/50">{{ file.type?.startsWith('video/') ? '短视频' : '图片' }} · {{ formatBytes(file.size) }}</p>
              </div>
            </div>
            <button
              type="button"
              class="text-white/50 transition hover:text-brand-accent"
        ...
        <div class="flex flex-wrap items-center gap-2 text-xs text-white/70">
          ...
          <template v-if="selectedTags.length">
            ...
            <button type="button" class="text-white/50 hover:text-white" @click="clearSelectedTags">清空</button>
          </template>
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <button
            type="button"
            class="flex items-center justify-center gap-2 rounded-2xl border border-white/10 px-5 py-3 text-sm font-semibold text-white/80 transition hover:border-white hover:text-white"
            @click="openTagDialog"
          >
            <Tag class="h-4 w-4" />
            添加标签
      ...
      <div v-if="uploadResult.length" class="space-y-4 rounded-3xl border border-brand-primary/20 bg-brand-primary/5 p-5">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <button type="button" class="text-sm text-white/70 transition hover:text-white" @click="clearResults">
            隐藏结果
          </button>
        </div>
        <ul class="space-y-3">
          <li
            ...
            :key="item.objectKey"
            class="rounded-2xl border border-white/10 bg-surface-body/50 px-4 py-3"
          >
            <div class="flex flex-col gap-1 text-left">
              <p class="text-sm font-semibold text-white">
                {{ item.originalFileName || item.fileName }}
                <span v-if="item.mediaCategory === 'VIDEO'" class="ml-2 rounded-full border border-white/15 px-2 py-0.5 text-xs text-white/70">短视频</span>
              </p>
              <p class="text-xs text-white/60">{{ item.fileName }} · {{ formatBytes(item.size) }}</p>
            </div>
            <div v-if="item.tags?.length" class="mt-2 flex flex-wrap gap-2 text-xs text-white/70">
              <span
            ...
            <div class="mt-3 flex flex-wrap gap-3 text-sm">
              <a
                ...
                target="_blank"
                rel="noopener"
                class="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-white/80 transition hover:border-white hover:text-white"
              >
                查看链接
              </a>
              <button
                type="button"
                class="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-white transition hover:bg-white/20"
