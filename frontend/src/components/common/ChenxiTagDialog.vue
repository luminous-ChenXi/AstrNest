...
const reportSuspiciousInput = (context, payload) => {
  console.warn(`[security] 阻止可疑输入(${context}):`, payload)
}
...
const toggleTag = (name) => {
  ...
  if (normalized.length > MAX_TAG_NAME_LENGTH) {
    ElMessage.warning(`标签名称不可超过 ${MAX_TAG_NAME_LENGTH} 个字符`)
    return
  }
  if (hasDangerousChars(normalized)) {
    reportSuspiciousInput('tag-select', normalized)
    ElMessage.error('检测到非法标签，已阻止选择')
    return
  }
...
const loadTags = async () => {
  ...
  try {
    ...
    if (searchTerm.length > MAX_TAG_NAME_LENGTH) {
      ElMessage.warning(`搜索关键字不可超过 ${MAX_TAG_NAME_LENGTH} 个字符`)
      loading.value = false
      return
    }
    if (searchTerm && hasDangerousChars(searchTerm)) {
      reportSuspiciousInput('tag-search-dialog', searchTerm)
      ElMessage.error('检测到非法字符，已阻止搜索')
      loading.value = false
      return
    }
    keyword.value = searchTerm
    const data = await searchTags({ keyword: searchTerm || undefined, limit: 30 })
...
const handleCreate = async () => {
  ...
  if (hasDangerousChars(normalized)) {
    reportSuspiciousInput('tag-create', normalized)
    ElMessage.error('检测到可疑字符，已阻止提交')
    newTagName.value = ''
    return
  }
  if (isSelected(normalized)) {
