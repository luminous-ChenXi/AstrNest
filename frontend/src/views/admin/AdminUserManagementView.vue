...
<template>
  <section class="space-y-6">
    <header>
      <p class="text-sm uppercase tracking-[0.4em] text-white/60">members</p>
      <h1 class="text-4xl font-semibold text-gradient">用户管理</h1>
      <p class="mt-2 text-white/70">
        列表视图展示全部成员，支持搜索 / 调整角色 / 配额与删除操作，默认新注册用户享有每日 100 张、空间 200MB 配额。
      </p>
    </header>

    <div class="table-shell glass-panel rounded-3xl border border-white/10 bg-black/30 p-6">
...
.table-shell :deep(.el-input__wrapper),
.table-shell :deep(.el-select .el-input__wrapper),
.table-shell :deep(.el-input-number .el-input__wrapper) {
...
.table-shell :deep(.el-input__inner),
.table-shell :deep(.el-input-number__inner) {
  color: #fff;
}

.table-shell :deep(.el-select-dropdown) {
...
.user-table-surface {
  --table-surface: #050505;
  --table-header: rgba(15, 23, 42, 0.92);
  --table-border: rgba(255, 255, 255, 0.08);
  --table-hover: rgba(255, 255, 255, 0.06);
  --table-text: rgba(248, 250, 252, 0.92);
...
.user-table-surface :deep(.el-table__inner-wrapper),
.user-table-surface :deep(.el-table__body-wrapper) {
  background: transparent;
}

.user-table-surface :deep(.el-table__header),
.user-table-surface :deep(.el-table__header-wrapper),
.user-table-surface :deep(.el-table__header thead),
.user-table-surface :deep(.el-table__header tr) {
  background: var(--table-header);
}

.user-table-surface :deep(.el-table__inner-wrapper::before),
.user-table-surface :deep(.el-table__inner-wrapper::after),
.user-table-surface :deep(.el-table__border-left-patch) {
  background: transparent;
}

.user-table-surface :deep(.el-table__header-wrapper th),
.user-table-surface :deep(.el-table__header th),
.user-table-surface :deep(.el-table-fixed-column--right th) {
  background: var(--table-header) !important;
  color: rgba(248, 250, 252, 0.88);
  border-color: var(--table-border) !important;
}

.user-table-surface :deep(.el-table__header-wrapper .cell),
.user-table-surface :deep(.el-table__header .cell) {
...
.user-table-surface :deep(td) {
  background-color: transparent !important;
  color: var(--table-text);
  border-color: var(--table-border);
}

.user-table-surface :deep(.el-table__body tr:nth-child(even) > td) {
