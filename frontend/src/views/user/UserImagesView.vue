...
<template>
  <div class="user-images space-y-8">
    <section class="glass-panel rounded-[32px] border border-white/10 bg-white/5 p-6">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-white/60">批量查看、复制或删除你的上传记录</p>
          <div v-if="upload.limits && upload.limits.maxFilesPerDay" class="mt-2 text-xs text-white/50">
        ...
        <div class="flex flex-wrap items-center gap-3 text-xs text-white/70">
          <span class="rounded-full border border-white/20 px-3 py-1">共 {{ total }} 张</span>
          <span v-if="selectedCount" class="rounded-full border border-brand-primary/40 px-3 py-1 text-brand-primary">
            已选择 {{ selectedCount }} 张
          </span>
          <button
            class="rounded-full border border-rose-400/40 px-4 py-2 text-sm font-semibold text-rose-200 transition hover:border-rose-300 hover:text-white disabled:opacity-50"
      ...
      <p v-if="message" class="mt-4 text-sm" :class="message.includes('失败') ? 'text-brand-accent' : 'text-brand-emerald'">{{ message }}</p>
    </section>

    <section>
      <div v-if="loading" class="rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/60">加载中…</div>
      <div v-else-if="!items.length" class="rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/60">暂无图片，快去上传吧</div>
      <div v-else class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        <article
          v-for="item in items"
          :key="item.id"
          class="glass-panel flex flex-col gap-4 rounded-3xl border border-white/10 bg-black/30 p-4 shadow-card"
        >
          <div class="relative overflow-hidden rounded-2xl">
            ...
            <label class="absolute left-3 top-3 inline-flex items-center gap-2 rounded-full bg-black/50 px-3 py-1 text-xs">
              <input
                type="checkbox"
                class="h-4 w-4 rounded border-white/30 bg-transparent text-brand-primary focus:ring-brand-primary/40"
          ...
          <div class="space-y-2 text-sm">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 flex-1">
                <p class="font-semibold text-white truncate" :title="item.fileName">{{ item.fileName }}</p>
                <p class="text-xs text-white/60">{{ (item.size / 1024).toFixed(1) }} KB · {{ item.reviewStatus || '已发布' }}</p>
              </div>
              <div class="min-w-[7rem] shrink-0 text-right text-xs text-white/60">
                <p>状态：{{ item.publicAccessible ? '公开' : '仅自己' }}</p>
                <p>喜欢：{{ item.likeCount || 0 }}</p>
              </div>
            </div>
            <div class="flex flex-wrap gap-2 text-xs text-white/60">
              <span class="rounded-full border border-white/20 px-2 py-1">调用 {{ item.invokeCount || 0 }}</span>
              <span v-if="item.violation" class="rounded-full border border-brand-accent/40 px-2 py-1 text-brand-accent">需复检</span>
            </div>
          </div>
          <div class="flex gap-3 text-sm font-semibold">
            <button
              class="flex-1 rounded-2xl bg-gradient-to-r from-brand-primary to-brand-accent px-3 py-2 text-sm shadow-[0_10px_30px_rgba(127,123,255,0.35)] transition hover:translate-y-0.5 disabled:opacity-60"
              type="button"
            ...
            <button
              class="flex-1 rounded-2xl border border-rose-400/40 px-3 py-2 text-sm text-rose-200 transition hover:border-rose-300 hover:text-white disabled:opacity-50"
              type="button"
    ...
    <div v-if="totalPages() > 1" class="flex items-center justify-center gap-4 text-sm">
      <button class="rounded-full border border-white/15 px-4 py-2 text-white/70 disabled:opacity-40" :disabled="page === 0" @click="goPage(page - 1)">
        上一页
      </button>
      <span class="text-white/60">{{ page + 1 }} / {{ totalPages() }}</span>
      <button class="rounded-full border border-white/15 px-4 py-2 text-white/70 disabled:opacity-40" :disabled="page >= totalPages() - 1" @click="goPage(page + 1)">
        下一页
      </button>
    </div>
  </div>
...
<style scoped>
.user-images :deep(.text-white\/80) {
  color: var(--text-body-secondary);
}

.user-images :deep(.text-white\/70) {
  color: var(--text-body-muted);
}

.user-images :deep(.text-white\/60) {
  color: var(--text-body-soft);
}

.user-images :deep(.text-white\/50),
.user-images :deep(.text-white\/40) {
  color: var(--text-body-faint);
}

.user-images :deep(.text-white) {
  color: var(--color-text-primary);
}

.user-images :deep(.bg-white\/5) {
  background-color: var(--panel-overlay) !important;
}

.user-images :deep(.bg-black\/30),
.user-images :deep(.bg-black\/20) {
  background-color: var(--color-bg-strong) !important;
}

.user-images :deep(.bg-black\/60) {
  background-color: rgba(0, 0, 0, 0.55) !important;
}

.user-images :deep(.border-white\/10),
.user-images :deep(.border-white\/15),
.user-images :deep(.border-white\/20),
.user-images :deep(.border-white\/30) {
