...
import { fetchOverviewMetrics, fetchOperationLogs, fetchSecuritySummary } from '../services/monitoring'
...
const fetchData = async () => {
  ...
  errorMessage.value = ''
  try {
    const [overview, logEntries, security] = await Promise.all([
  ...
  <section class="space-y-6">
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      ...
      <article class="glass-panel p-6">
        ...
        <ul class="mt-4 space-y-3 max-h-[320px] overflow-y-auto pr-1">
          <li
            v-for="log in logs"
            :key="log.id"
      ...
      <article class="glass-panel p-6 space-y-4">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-white/60 text-sm">安全看板</p>
            <h2 class="text-2xl font-semibold">登录 / 注册风险</h2>
          </div>
          <button class="text-sm text-brand-primary" @click="fetchData">刷新</button>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 text-sm text-white">
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs">24 小时登录错误</p>
            <p class="text-2xl font-semibold mt-1">{{ securitySummary.loginFailLast24h ?? '--' }}</p>
          </div>
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs">累计登录错误</p>
            <p class="text-2xl font-semibold mt-1">{{ securitySummary.loginFailTotal ?? '--' }}</p>
          </div>
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs">24 小时注册错误</p>
            <p class="text-2xl font-semibold mt-1">{{ securitySummary.registerFailLast24h ?? '--' }}</p>
          </div>
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs">风险 IP 数</p>
            <p class="text-2xl font-semibold mt-1">{{ securitySummary.riskIps?.length ?? 0 }}</p>
          </div>
        </div>

        <div class="grid md:grid-cols-2 gap-3 text-sm">
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs mb-2">登录用户名 Top</p>
            <ul class="space-y-1 max-h-36 overflow-y-auto">
              <li v-for="item in securitySummary.topLoginUsernames || []" :key="item.key" class="flex justify-between text-white/80">
                <span>{{ item.key }}</span>
                <span class="text-white/60">{{ item.count }}</span>
              </li>
              <li v-if="!(securitySummary.topLoginUsernames?.length)" class="text-white/50 text-xs">暂无数据</li>
            </ul>
          </div>
          <div class="rounded-2xl bg-white/5 border border-white/10 p-3">
            <p class="text-white/60 text-xs mb-2">注册用户名 Top</p>
            <ul class="space-y-1 max-h-36 overflow-y-auto">
              <li v-for="item in securitySummary.topRegisterUsernames || []" :key="item.key" class="flex justify-between text-white/80">
                <span>{{ item.key }}</span>
                <span class="text-white/60">{{ item.count }}</span>
              </li>
              <li v-if="!(securitySummary.topRegisterUsernames?.length)" class="text-white/50 text-xs">暂无数据</li>
            </ul>
          </div>
        </div>

        <div>
          <p class="text-white/60 text-xs mb-2">风险 IP（滚动）</p>
          <div class="max-h-36 overflow-y-auto space-y-2 pr-1">
            <div
              v-for="ip in securitySummary.riskIps || []"
              :key="ip.ip + (ip.lockedUntil || '') + ip.dimension"
              class="rounded-xl bg-white/5 border border-white/10 px-3 py-2 text-white/80"
            >
              <div class="flex justify-between text-sm">
                <span>{{ ip.ip || '未知IP' }}</span>
                <span class="text-white/60 text-xs">{{ ip.dimension }}</span>
              </div>
              <p class="text-xs text-white/60 mt-1 truncate">{{ ip.reason || '已锁定' }}</p>
              <p class="text-xs text-amber-200">{{ ip.lockedUntil ? `解封于 ${new Date(ip.lockedUntil).toLocaleString('zh-CN',{hour12:false})}` : '' }}</p>
            </div>
            <p v-if="!(securitySummary.riskIps?.length)" class="text-white/50 text-xs">当前暂无风险 IP</p>
          </div>
        </div>
      </article>
    </div>
