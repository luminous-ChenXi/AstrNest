...
import com.imgbed.chenxi.captcha.ChenxiCaptchaImageFactory.CaptchaImagePayload;
...
public class ChenxiCaptchaService {
  ...
  public ChenxiCaptchaVerifyResponse verifyChallenge(ChenxiCaptchaVerifyRequest request) {
    ...
    ChenxiCaptchaTicket ticket = ticketRepository.findById(request.captchaId())
    ...
    String submitted = request.captchaCode().trim();
  ...
  public void consumeCertificationOrThrow(String certificationToken) {
    ChenxiCaptchaTicket ticket = ticketRepository.findByVerificationToken(certificationToken)
